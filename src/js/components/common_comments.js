...
class CommonComments extends React.Component {
	...
	handleCollect(){
		...
		fetch("http://newsapi.gugujiankong.com/Handler.ashx?action=comment&userid="+localStorage.userid+ "&uniquekey" +this.props.uniquekey,myFetchOptions)		
		.then(response=>response.json())
		.then(json=>{
			//全局提醒收藏成功
			notification['success']({message:'React News提醒',description:'收藏此文章成功'});
		});
	};
				...
				<Row>
					<Col span={24}>
						...
						<Form onSubmit ={this.handleSubmit.bind(this)}>
							...
							&nbsp;&nbsp;
							<Button type="primary" htmlType="button" onClick={this.handleCollect.bind(this)}>收藏此文章</Button>
						</Form>
					</Col>	
				</Row>
			</div>
